...
public class AutoCompleteComboBox<T> implements EventHandler<KeyEvent> {
    ...
    private void selectClosestResultBasedOnTextFieldValue(boolean affect, boolean inFocus) {
        ...
        boolean found = false;
        for (int i=0; i<items.size(); i++) {
            if (AutoCompleteComboBox.this.comboBox.getEditor().getText().toLowerCase().equals(items.get(i).toString().toLowerCase())) {
                try {
        ...
        if (!found && affect) {     System.out.println("Okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkm");       
...
//            KeyEvent ke = new  KeyEvent.impl_keyEvent(comboBox, KeyCode.ENTER.toString(), KeyCode.ENTER.getName(), KeyCode.ENTER.impl_getCode(),
//                                                    false, false, false, false, KeyEvent.KEY_RELEASED);
//            comboBox.fireEvent(KeyEvent.);
        KeyEvent   keyEvent = new KeyEvent(comboBox, null, KeyEvent.KEY_PRESSED, KeyCode.ENTER.toString(),
        KeyCode.ENTER.getName(), KeyCode.DOWN, false, false, false, false); 
        comboBox.fireEvent(keyEvent);
        }
    }
